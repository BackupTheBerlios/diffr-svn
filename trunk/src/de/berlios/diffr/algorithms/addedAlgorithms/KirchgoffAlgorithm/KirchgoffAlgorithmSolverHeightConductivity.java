...
public class KirchgoffAlgorithmSolverHeightConductivity extends
	...
	public SurfaceCurrent calculateSurfaceCurrent(int numberOfPoints) {
		...
		for (int j = 0; j< numberOfPoints; j++){
			if (polarization == IncidentWave.polarizationE) {
				current[j] = normalDerivativeOfIncidentField(xCurrent);//Overwrite!!!  not correct
			} else {
				current[j] = incidentField(xCurrent,f_surface(xCurrent));//Overwrite!!!  not correct
			}
			xCurrent += deltaX; 
		}
