...
public class Algorithm2ForVerySimpleTask extends Algorithm {
  private long sleepTime;
  private double validity;
  public Algorithm2ForVerySimpleTask(Long sleepTime, Double validity) {
  ...
  public TaskResults solve(Task task) throws ErrorInAlgorithmException {
    ...
    try {
      if (validity == 0) throw new ErrorInAlgorithmException();
      double angle = ((VerySimpleImpingingField)task.getImpingingField()).getAngle();
      field = new VerySimpleReflectedField(new Double(Math.PI - angle / validity));
    }
    catch (Exception ex) {
